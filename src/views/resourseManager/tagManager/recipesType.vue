...
<script>
	import { fetchFoodType, getRelationFoodCategory, insertFoodType, delFoodType, updateFoodType, updateFoodTypeStatus,fetchRecipeType } from "@/api/foodManager";
	...
	export default {
		...
		methods: {
			...
			tableLazyLoad(tree, treeNode,resolve) {
				...
				getCookCategoryList(tmp).then(res => {
					var data = res.data;
					for(let i in data) {
...
//				console.log(row)
			...
			getList: function() {
				...
				getCookCategoryList(param).then(res => {
					this.list = res.data
