          ...
          <el-col :span="8">
            ...
            >
              <div>
                ...
                <!--</div>-->
                <el-select style="width: 200px;" filterable v-model="postForm.condition">
                  <el-option
        ...
        <el-row>
          <el-col :span="20" v-if="relationType != 2">
            <el-form-item
              ...
              class="postInfo-container-item"
            >
              <el-select v-model="postForm.relation_value"
                         remote
                         filterable
        ...
        <el-row>
          <el-col :span="20">
            <el-form-item
              ...
              class="postInfo-container-item"
            >
              <el-checkbox v-model="isAllSelected" :disabled="isDisabled">全选</el-checkbox>
...
export default {
  ...
  data() {
    const validateTime = (rule, value, callback) => {
      if (value.length <= 0 && this.postForm.type == 1) {
  ...
  watch: {
    recommend(value) {
      this.userFeatureList = this.userFeatureList.map(item => {
        return { value: item.value, name: item.name, select: !item.select }
      })
      ...
      this.$refs['postForm'].validate(valid => {
        if (valid) {
          ...
          if (this.postForm.type === 1) {
            this.postForm.condition = this.holidayCondition
            this.postForm.side_condition = this.holidayConditionDate
          }
          createItem(this.postForm).then(res => {
